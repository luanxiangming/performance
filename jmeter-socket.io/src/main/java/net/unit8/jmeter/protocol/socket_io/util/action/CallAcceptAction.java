...
public class CallAcceptAction extends SocketIOAsyncActionBase {

    public CallAcceptAction(int userIndex) {
        super(userIndex);
    }
    
    public static final String SEND_MESSAGE = "{\"type\":\"call.accept\"}";
    ...
    public SocketIOCallbackData[] run(SocketIOCallbackData[] datas,
        ...
        final CallAcceptAction parent = this;
                    ...
                    public void on(String event, IOAcknowledge ack, JsonElement... args) {
                        ...
                        synchronized (parent) {
                            if (event.equals(SEND_EVENT) &&
                                    args[0].getAsJsonObject().get("type").getAsString().equals("call.join")) {
                                data.event = event;
                                data.eventAck = ack;
                                data.eventArgs = args;
                                receivedNum ++;
        ...
        if (receivedNum != userNum - 1) {
            throw new Exception("Call accept not received.");
        }
        return datas;
    }
